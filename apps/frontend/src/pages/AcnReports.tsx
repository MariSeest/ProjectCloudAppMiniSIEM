...
export default function AcnReports() {
    ...
    const [open, setOpen] = useState<Record<string, boolean>>({
        ...
        F: false, G: false, H: false, I: false, L: false,
    })
    ...
    async function save(submit = false) {
        ...
        try {
            const payload = {
                ...
                sectionA: sA, sectionB: sB, sectionC: sC, sectionD: sD,
                sectionE: sE, sectionF: sF, sectionG: sG, sectionH: sH,
                sectionI: sI, sectionL: sL,
            }
            let report
    ...
    const upd = (setter: React.Dispatch<React.SetStateAction<any>>, key: string, val: string) =>
        ...
        <div className="page">
            ...
            <Section title="SEZIONE A — Dati dell'Organizzazione" open={open.A} onToggle={() => toggle('A')}>
                <div className="form-grid">
                    <F label="Denominazione Sociale" required>
                        <input className="input" value={sA.denominazione || ''} onChange={(e) => upd(setSA, 'denominazione', e.target.value)} />
                    </F>
                    <F label="Tipologia di soggetto" required>
                        <input className="input" value={sA.tipologia || ''} onChange={(e) => upd(setSA, 'tipologia', e.target.value)} placeholder="Essenziale / Importante" />
                    </F>
                    <F label="Codice Fiscale" required>
                        <input className="input" value={sA.codiceFiscale || ''} onChange={(e) => upd(setSA, 'codiceFiscale', e.target.value)} />
                    </F>
                    <F label="Partita IVA">
                        <input className="input" value={sA.piva || ''} onChange={(e) => upd(setSA, 'piva', e.target.value)} />
                    </F>
                    <F label="Codice NIS (ENISA)" required>
                        <input className="input" value={sA.codiceNIS || ''} onChange={(e) => upd(setSA, 'codiceNIS', e.target.value)} />
                    </F>
                    <F label="Domicilio digitale (PEC)" required>
                        <input className="input" value={sA.pec || ''} onChange={(e) => upd(setSA, 'pec', e.target.value)} placeholder="ufficio@pec.example.it" />
                    </F>
                </div>
            ...
            <Section title="SEZIONE B — Dati del Segnalante e Referente CSIRT" open={open.B} onToggle={() => toggle('B')}>
                <div className="form-grid">
                    <F label="Nome e Cognome" required>
                        <input className="input" value={sB.nome || ''} onChange={(e) => upd(setSB, 'nome', e.target.value)} />
                    </F>
                    <F label="Ruolo e funzioni">
                        <input className="input" value={sB.ruolo || ''} onChange={(e) => upd(setSB, 'ruolo', e.target.value)} />
                    </F>
                    <F label="Email PEC/PEO" required>
                        <input className="input" value={sB.email || ''} onChange={(e) => upd(setSB, 'email', e.target.value)} />
                    </F>
                    <F label="Recapito telefonico" required>
                        <input className="input" value={sB.telefono || ''} onChange={(e) => upd(setSB, 'telefono', e.target.value)} />
                    </F>
                </div>
            </Section>
            ...
            <Section title="SEZIONE E — Descrizione Impatto" open={open.E} onToggle={() => toggle('E')}>
                <div className="form-grid">
                    <F label="Numero utenti impattati">
                        <input className="input" type="number" value={sE.utentiImpattati || ''} onChange={(e) => upd(setSE, 'utentiImpattati', e.target.value)} />
                    </F>
                    <F label="Percentuale utenti impattati">
                        <input className="input" type="number" max="100" value={sE.percentualeUtenti || ''} onChange={(e) => upd(setSE, 'percentualeUtenti', e.target.value)} placeholder="%" />
                    </F>
                    <F label="Data inizio interruzione">
                        <input className="input" type="datetime-local" value={sE.inizioInterruzione || ''} onChange={(e) => upd(setSE, 'inizioInterruzione', e.target.value)} />
                    </F>
                    <F label="Data fine interruzione">
                        <input className="input" type="datetime-local" value={sE.fineInterruzione || ''} onChange={(e) => upd(setSE, 'fineInterruzione', e.target.value)} />
                    </F>
                    <F label="Durata disservizio (minuti)">
                        <input className="input" type="number" value={sE.durata || ''} onChange={(e) => upd(setSE, 'durata', e.target.value)} />
                    </F>
        ...
        <div className="page">
            ...
            <div className="card">
                <div className="card-head">
                    <span className="card-title">Report ACN</span>
                    <span className="card-count">{reports.length}</span>
                </div>
                {loading
                        ...
                        {reports.length === 0
                            ...
                            : reports.map((r: any) => (
                                <tr key={r.id}>
                                    ...
                                    <td>
                        <span className={`badge ${r.status === 'SUBMITTED' ? 'badge-resolved' : r.status === 'DRAFT' ? 'badge-medium' : 'badge-closed'}`}>
                          {r.status}
                        </span>
                                    </td>
